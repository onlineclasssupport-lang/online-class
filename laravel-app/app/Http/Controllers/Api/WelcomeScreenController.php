...
class WelcomeScreenController extends Controller
{
    ...
    public function getSettings()
    {
        ...
        if (!$dashboardLogoPath || !Storage::disk('public')->exists($dashboardLogoPath)) {
            $eduPath = AdminSetting::where('key', 'education_logo_path')->value('value');
            if ($eduPath && Storage::disk('public')->exists($eduPath)) {
                $dashboardLogoPath = $eduPath;
            }
        }
        $dashboardLogoUrl = null;
        if ($dashboardLogoPath && Storage::disk('public')->exists($dashboardLogoPath)) {
            $dashboardLogoUrl = url('api/site-settings/logo');
        }
    ...
    public function updateSettings(Request $request)
    {
        ...
        if ($request->boolean('remove_dashboard_logo')) {
            ...
            $eduPath = AdminSetting::where('key', 'education_logo_path')->value('value');
            if ($eduPath && Storage::disk('public')->exists($eduPath)) {
                Storage::disk('public')->delete($eduPath);
            }
            ...
            $origName = $logoFile->getClientOriginalName();
            ...
            AdminSetting::updateOrCreate(['key' => 'education_logo_path'], ['value' => $newLogoPath]);
